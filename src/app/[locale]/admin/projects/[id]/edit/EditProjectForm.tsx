    ...
    <div>
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          ...
          <Field label="Categoría">
            <select name="category" value={form.category} onChange={handleChange} style={inputStyle}>
              {CATEGORIES.map((c) => (
                <option key={c} value={c}>{c === "cliente" ? "Cliente" : "Demo"}</option>
              ))}
            </select>
          </Field>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          ...
          <Field label="Destacado">
            <label style={{ display: "flex", alignItems: "center", gap: "8px", padding: "12px 0" }}>
              ...
              <span style={{ fontSize: "14px", color: "var(--muted)" }}>Mostrar en Home</span>
            </label>
          </Field>
        </div>
        ...
        <Field label="Problema">
          <textarea name="problem_es" value={form.problem_es} onChange={handleChange} style={textAreaStyle} required />
        </Field>
        <Field label="Solución">
          <textarea name="solution_es" value={form.solution_es} onChange={handleChange} style={textAreaStyle} required />
        </Field>
        ...
        <Field label="Problem">
          <textarea name="problem_en" value={form.problem_en} onChange={handleChange} style={textAreaStyle} required />
        </Field>
        <Field label="Solution">
          <textarea name="solution_en" value={form.solution_en} onChange={handleChange} style={textAreaStyle} required />
        </Field>

        <Field label="Tecnologías (separadas por coma)">
          <input name="technologies" value={form.technologies} onChange={handleChange} style={inputStyle} />
        </Field>
